...
/*   Updated: 2019/09/28 16:19:21 by eklompus         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_result	read_dir(t_lsdata *lsd, t_fentry *fentry)
{
	DIR				*dir;
	struct dirent	*dd;
	t_list			*lst;
	...
	while ((dd = readdir(dir)) != NULL)
	{
		...
		if (stat(ffentry->name, &fentry->fs) != 0)
			return (ERR_STAT);
		ft_lstaddsorted(&fentry->child, lst, &(lsd->flags), f_cmp);
	}
	closedir(dir);
	return (RET_OK);
}
